...
const CardNav: React.FC<CardNavProps> = ({
    ...
    return (
        ...
        >
            <nav
                ref={navRef}
                className={`card-nav ${isExpanded ? 'open' : ''} block h-[60px] p-0 rounded-xl border border-[var(--border-color)] relative overflow-hidden will-change-[height]`}
                        ...
                        >
                            ...
                            <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                                {item.links?.map((lnk, i) => {
                                    ...
                                    const className = "nav-card-link inline-flex items-center gap-[6px] no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] text-left bg-transparent border-0 p-0 font-inherit w-full font-normal";
                                    const content = (
                                        <>
                                    ...
                                    if (lnk.href) {
                                        ...
                                        return (
                                            <Link key={`${lnk.label}-${i}`} href={lnk.href} className={className} onClick={lnk.onClick} aria-label={lnk.ariaLabel}>
                                                {content}
                                            </Link>
                                        );
                                    }
                                    return (
                                        <button key={`${lnk.label}-${i}`} type="button" className={className} onClick={lnk.onClick} aria-label={lnk.ariaLabel}>
                                            {content}
                                        </button>
                                    );
                                })}
