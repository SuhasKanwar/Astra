...
const CardNav: React.FC<CardNavProps> = ({
    ...
    return (
        ...
        >
            <nav
                ref={navRef}
                className={`card-nav ${isExpanded ? 'open' : ''} block h-15 p-0 rounded-xl border border-(--border-color) relative overflow-hidden will-change-[height]`}
                        ...
                        >
                            ...
                            <div className="nav-card-links mt-auto flex flex-col gap-0.5">
                                {item.links?.map((lnk, i) => {
                                    ...
                                    const className = "nav-card-link inline-flex items-center gap-1.5 no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] text-left bg-transparent border-0 p-0 font-inherit w-full font-normal";
                                    const content = (
                                        <>
